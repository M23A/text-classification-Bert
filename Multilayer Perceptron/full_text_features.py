...
X_combined = data.drop(['Label', 'PID'], axis=1).values
y_combined = data['Label'].values  # already 0 and 1
# Define person_ids
person_ids = data['PID']
...
for train_idx, test_idx in gkf.split(X_combined, y_combined, groups=person_ids):
    ...
    mlp = MLPClassifier(hidden_layer_sizes=(512,), max_iter=200, activation='relu', learning_rate_init=0.01, random_state=42)
